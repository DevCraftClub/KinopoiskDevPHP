...
class PersonSearchFilterTest extends TestCase {
	...
	public function test_age_filter_with_default_operator(): void {
		...
		$filters = $this->filter->getFilters();
		$this->assertArrayHasKey('age', $filters);
		$this->assertEquals(25, $filters['age']);
	}

	public function test_sex_filter(): void {
		$this->filter->sex('male');
		$filters = $this->filter->getFilters();
		$this->assertArrayHasKey('sex', $filters);
		$this->assertEquals('male', $filters['sex']);
	}

	public function test_birthPlace_filter(): void {
		$this->filter->birthPlace('Москва', 'regex');
		$filters = $this->filter->getFilters();
		$this->assertArrayHasKey('birthPlace.value', $filters);
		$this->assertEquals('Москва', $filters['birthPlace.value']);
	}
	...
	public function test_birthPlace_filter_with_default_operator(): void {
		...
		$filters = $this->filter->getFilters();
		$this->assertArrayHasKey('birthPlace.value', $filters);
		$this->assertEquals('Санкт-Петербург', $filters['birthPlace.value']);
	}

	public function test_death_filter(): void {
		$this->filter->death('2020-01-01');
		$filters = $this->filter->getFilters();
		$this->assertArrayHasKey('death', $filters);
		$this->assertEquals('2020-01-01', $filters['death']);
	}
	...
	public function test_death_filter_with_default_operator(): void {
		...
		$filters = $this->filter->getFilters();
		$this->assertArrayHasKey('death', $filters);
		$this->assertEquals('2019-12-31', $filters['death']);
	}

	public function test_birthday_filter(): void {
		$this->filter->birthday('1980-05-15');
		$filters = $this->filter->getFilters();
		$this->assertArrayHasKey('birthday', $filters);
		$this->assertEquals('1980-05-15', $filters['birthday']);
	}
	...
	public function test_birthday_filter_with_default_operator(): void {
		...
		$filters = $this->filter->getFilters();
		$this->assertArrayHasKey('birthday', $filters);
		$this->assertEquals('1975-03-20', $filters['birthday']);
	}

	public function test_countAwards_filter(): void {
		$this->filter->countAwards(5);
		$filters = $this->filter->getFilters();
		$this->assertArrayHasKey('countAwards', $filters);
		$this->assertEquals(5, $filters['countAwards']);
	}
	...
	public function test_countAwards_filter_with_default_operator(): void {
		...
		$filters = $this->filter->getFilters();
		$this->assertArrayHasKey('countAwards', $filters);
		$this->assertEquals(10, $filters['countAwards']);
	}

	public function test_profession_filter(): void {
		$this->filter->profession('актер');
		$filters = $this->filter->getFilters();
		$this->assertArrayHasKey('profession', $filters);
		$this->assertEquals('актер', $filters['profession']);
	}
	...
	public function test_profession_filter_with_default_operator(): void {
		...
		$filters = $this->filter->getFilters();
		$this->assertArrayHasKey('profession', $filters);
		$this->assertEquals('режиссер', $filters['profession']);
	}
	...
	public function test_onlyWriters_filter(): void {
		...
		$filters = $this->filter->getFilters();
		$this->assertArrayHasKey('profession', $filters);
		$this->assertEquals('сценарист', $filters['profession']);
	}
	...
	public function test_onlyAlive_filter(): void {
		...
		$filters = $this->filter->getFilters();
		$this->assertArrayHasKey('death', $filters);
		$this->assertNull($filters['death']);
	}

	public function test_birthYear_filter_single_year(): void {
		$this->filter->birthYear(1980);
		$filters = $this->filter->getFilters();
		$this->assertArrayHasKey('birthYear', $filters);
		$this->assertEquals(1980, $filters['birthYear']);
	}
	...
	public function test_birthYear_filter_range(): void {
		...
		$filters = $this->filter->getFilters();
		$this->assertArrayHasKey('birthYear', $filters);
		$this->assertEquals([1970, 1980], $filters['birthYear']);
	}

	public function test_deathYear_filter(): void {
		$this->filter->deathYear(2020);
		$filters = $this->filter->getFilters();
		$this->assertArrayHasKey('deathYear', $filters);
		$this->assertEquals(2020, $filters['deathYear']);
	}
	...
	public function test_chaining_filters(): void {
		...
		$this->assertEquals('актер', $filters['profession']);
		$this->assertArrayHasKey('birthYear', $filters);
		$this->assertEquals([1980, 1990], $filters['birthYear']);
	}
	...
	public function test_inherited_methods_from_movie_filter(): void {
		...
		$this->assertEquals('Том Круз', $filters['name']);
	}
